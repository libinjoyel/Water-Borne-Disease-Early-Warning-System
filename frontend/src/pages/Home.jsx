...
import { statsAPI, riskAPI } from '../services/api';
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        setActiveZones(data.locations.total);
        setWaterIndex(100 - (data.risk.averageScore || 0));
        setReportsCount(data.incidents.total);
        setAvgTemp(data.weather.averages.temperature);
        setAvgHumidity(data.weather.averages.humidity);
      } catch {
        const { data } = await riskAPI.getOverview().catch(() => ({ data: null }));
        if (data) {
          setActiveZones(data.totalLocations);
          setWaterIndex(100 - data.summary.averageRiskScore);
        }
      }
    };
    fetchStats();
        ...
        <div className="flex-1 space-y-6 text-center lg:text-left">
          ...
          >
            ...
            <p className="text-[10px] text-slate-400 leading-tight">
              Drag slider to simulate water quality drop. Background canvas wave shifts color (Teal &rarr; Amber &rarr; Red) and velocity dynamically.
            </p>
          </motion.div>
        </div>
        ...
        >
          <svg className="w-full h-full max-h-[420px]" viewBox="0 0 500 500" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <motion.path
              ...
              stroke="#0D9488" strokeWidth="4" strokeLinecap="round" fill="none" opacity="0.8"
              animate={{ d: ["M 120,250 Q 185,210 250,250 T 380,250", "M 120,250 Q 185,290 250,250 T 380,250", "M 120,250 Q 185,210 250,250 T 380,250"] }}
              transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
            />
            <motion.path
              d="M 140,280 Q 200,310 260,280 T 360,280"
              stroke="#38BDF8" strokeWidth="3" strokeLinecap="round" fill="none" opacity="0.6"
              animate={{ d: ["M 140,280 Q 200,310 260,280 T 360,280", "M 140,280 Q 200,250 260,280 T 360,280", "M 140,280 Q 200,310 260,280 T 360,280"] }}
            ...
            >
              ...
              <motion.path
                ...
                stroke="#14B8A6" strokeWidth="3" strokeDasharray="4 4" fill="none"
                animate={{ rotate: 360 }}
